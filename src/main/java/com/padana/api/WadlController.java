...
public class WadlController {
	...
	public @ResponseBody Application generateWadl(HttpServletRequest request) {
		...
		for (Map.Entry<RequestMappingInfo, HandlerMethod> entry : handletMethods
			...
			boolean isRestContoller = bean.getClass()
			...
			for (RequestMethod httpMethod : httpMethods) {
				...
				wadlDocMethod
						.setTitle(javaMethod.getDeclaringClass().getSimpleName()
								+ "." + javaMethod.getName());
				...
				for (Annotation[] annotation : annotations) {
					...
					for (Annotation annotation2 : annotation) {
						...
						if (annotation2 instanceof RequestParam) {
							...
							String defaultValue = cleanDefault(
									param2.defaultValue());
				...
				if (!mediaTypes.isEmpty()) {
					...
					Class methodReturn = handlerMethod.getReturnType()
							.getClass();
					ResponseStatus status = handlerMethod
							.getMethodAnnotation(ResponseStatus.class);
					if (status == null) {
						wadlResponse.getStatus()
								.add((long) (HttpStatus.OK.value()));
					} else {
